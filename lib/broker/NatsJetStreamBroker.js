...
const { BehaviorSubject, from, defer, EMPTY, timer } = require('rxjs');
const { filter, map, timeout, first, switchMap, mapTo, mergeMap, reduce, mergeMapTo, defaultIfEmpty, tap } = require('rxjs/operators');
...
class NatsJetStreamBroker {
  ...
  start$(streamName, subjects = [], streamOptions = {}) {
    return defer(() => {
      return this.connect().then(async () => {
        console.log(`NATS JetStream client connected to ${this.natsServerUrl}`);
        // Create a stream if it doesn't exist
        // and if the subjects are provided
        if(
          streamName != null &&
          streamName !== "" &&
  ...
  /**
   * Establishes connection to the NATS server and creates a JetStream client.
   */
  async connect() {
  ...
  sendAndGetReply$({
    ...
    return this.ensureSubjectInStream$([responseSubject]).pipe(
      switchMap(() => 
        this.send$(subject, type, payload, subjects, ops).pipe(
          switchMap(messageId =>
